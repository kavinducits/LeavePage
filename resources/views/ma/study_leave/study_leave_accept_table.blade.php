        ...
        <div class="card-body p-0">
                            ...
                            @foreach ($acceptedApplications as $application)
                                <tr class="hoverable-row">
                                    ...
                                    <td class="text-center">
                                        <a href="{{ route('hodacademicestablishment.studyLeave.view', $application->id) }}"
            ...
            @else
                <div class="text-center py-5">
                    ...
                    <h5 class="text-muted">No Study Leave Applications Accepted</h5>
                    <p class="text-muted">There are no study leave applications currently accepted by you.</p>
                </div>
            @endif
        </div>
        @if ($acceptedApplications->count() > 0)
            <div class="card-footer bg-light">
                <div class="text-muted text-center">
                    <small><i class="fas fa-info-circle me-1"></i>Total Applications:
                        <strong>{{ $acceptedApplications->count() }}</strong></small>
                </div>
            </div>
        @endif
    </div>

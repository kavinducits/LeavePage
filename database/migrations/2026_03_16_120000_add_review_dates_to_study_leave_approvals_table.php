...
return new class extends Migration
{
    ...
    {
        Schema::table('study_leave_approvals', function (Blueprint $table) {
            if (!Schema::hasColumn('study_leave_approvals', 'ma_reviewed_date')) {
                $table->date('ma_reviewed_date')->nullable();
            }

            if (!Schema::hasColumn('study_leave_approvals', 'ma_finalized_date')) {
                $table->date('ma_finalized_date')->nullable();
            }

            if (!Schema::hasColumn('study_leave_approvals', 'registrar_reviewed_date')) {
                $table->date('registrar_reviewed_date')->nullable();
            }

            if (!Schema::hasColumn('study_leave_approvals', 'hod_reviewed_date')) {
                $table->date('hod_reviewed_date')->nullable();
            }

            if (!Schema::hasColumn('study_leave_approvals', 'vc_reviewed_date')) {
                $table->date('vc_reviewed_date')->nullable();
            }

            if (!Schema::hasColumn('study_leave_approvals', 'dean_reviewed_date')) {
